...
export const fetchSessions = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const r = await axios.get(`${API}/sessions`, { headers: H() });
      return r.data;
    } catch (e) {
...
export const createSession = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      const r = await axios.post(`${API}/sessions`, data, { headers: H() });
      return r.data;
    } catch (e) {
      return rejectWithValue(
        e.response?.data?.message || "Server not running — start the backend"
      );
    }
  }
);
...
export const endSession = createAsyncThunk(
  "sessions/end",
  async ({ id, aiNotes }, { rejectWithValue }) => {
    try {
...
export const deleteSession = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await axios.delete(`${API}/sessions/${id}`, { headers: H() });
      return id;
    } catch (e) {
      return rejectWithValue(e.response?.data?.message || "Failed");
    }
...
const sessionsSlice = createSlice({
  ...
  reducers: {
    ...
    setListening: (s, a) => {
      s.isListening = a.payload;
    },
    setLiveQuestion: (s, a) => {
      s.liveQuestion = a.payload;
    },
    addAnswer: (s, a) => {
      s.answers.unshift(a.payload);
    },
    clearLiveSession: (s) => {
    ...
    b.addCase(fetchSessions.pending, (s) => {
      ...
      .addCase(createSession.fulfilled, (s, a) => {
        if (a.payload?.session) {
          ...
          s.activeSession = a.payload.session;
        }
      })
      .addCase(endSession.fulfilled, (s, a) => {
        if (a.payload?.session) {
          const i = s.sessions.findIndex(
            (x) => x._id === a.payload.session._id
          );
          if (i >= 0) s.sessions[i] = a.payload.session;
        }
      })
